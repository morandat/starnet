package fr.labri.starnet.policies.commons;

public class CommonVar {
	public static final String SAVED_MAILBOX = "saved_mailbox";
	public static final String CURRENT_MESSAGE = "current";
	public static final String FORWARDED_MESSAGE_SET = "forwarded_messages";
	public static final String HELLO_SET = "hello_set";
	public static final String DATA_SET = "hello_set";
	public static final String TTL = "ttl";
	public static final Long TIMEOUT = (long) 25;
}
